...
판정은 같고, 삭제 판정만 다릅니다: 서버가 그 사이트에 직접 요청을 보낼 수 없어 개별 페이지로
...
**⚠️ 이 방식이 통하는 건 Cloudflare의 세션(쿠키) 기반 챌린지일 때만입니다.** 사이트에 따라
서버 IP 자체를 네트워크 단에서 완전히 막아버리는(TCP 연결 자체가 타임아웃되는) 경우가 있는데,
...
2. 해당 사이트들(`sites.arcalive`, `sites.fmkorea`)은 **`enabled: true`로 켜둔 채** 유지하세요
   ...
   코드가 있음)는 Cloudflare에 계속 막혀 매 사이클 실패하지만, 그 자체는 무해합니다
   (기존 `failure_alert_threshold` 알림만 따로 옵니다) — 신경 쓰이면 `interval_seconds`를
   크게(예: 3600) 잡아 요청 빈도만 줄이세요.
