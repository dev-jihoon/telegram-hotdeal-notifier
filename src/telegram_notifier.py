...
def build_markup(
    ...
    admin_contacts: list[AdminContact] | None = None,
    ...
    rows = [buttons]
    # 관리자 문의 버튼은 본문 버튼과 한 줄에 같이 넣으면 좁아지니 별도 줄로 뺀다.
    for contact in admin_contacts or []:
        rows.append([InlineKeyboardButton(contact.label, url=f"https://t.me/{contact.username}")])
...
class TelegramNotifier:
    def __init__(
        ...
        # 값을 복사해두지 않고 설정 객체/리스트 자체를 들고 있는다 - 관리자가 텔레그램에서
        # /settings나 /admins로 값을 바꾸면(같은 객체를 그대로 수정) 재시작 없이 바로 다음
        # 메시지부터 반영된다.
